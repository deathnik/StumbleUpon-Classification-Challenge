...
object Main {
  ...
  def splittingFunction(text: String): Array[String] = text.toLowerCase().split("[^a-zA-Z-]").filter(s => s.length > 2)
  ...
  def testClassifier(mainData: List[Array[String]], testFilter: Array[String] => Boolean) {
    ...
    val  splitedData = mainData.splitAt((mainData.length * 0.7).toInt)
    val classif = new BayesTF_IDF(alchemeryCategorySet, s => splittingFunction(s))
  ...
  def classifyAndOutputResult(trainData: List[Array[String]]) {
    ...
    for (line <- testDate)
      output.write(line(1) + "," + (classif.classify(line) * 1000).round / 1000.0 + "\n")
